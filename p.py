...
def Gen_Eff(V, CL, IL, K, Rsh, Ra):
    ...
    # Calculate Core-Used Losses (CUL) in watts
    CUL = Ish**2 * Rsh + Ia**2 * Ra
    
    # Calculate Efficiency (Eff)
    Eff = ((K * V * IL - CL - CUL) / (K * V * IL)) * 100
    
    # Convert Core-Used Losses from watts to kilowatts
    CUL_kW = CUL / 1000
    
    return Eff, CUL_kW

# Streamlit UI
st.title("2305A21L41-CALCULATE THE EFFICIENCY OF DC SERIES GENERATOR")  # Title with Roll No. and Problem Statement No.
...
if st.button("Calculate Efficiency"):
    ...
    if V > 0 and CL >= 0 and IL > 0 and K > 0 and Rsh > 0 and Ra > 0:
        Eff, CUL_kW = Gen_Eff(V, CL, IL, K, Rsh, Ra)
        st.write(f"Efficiency: {Eff:.2f}%")
        st.write(f"Core-Used Losses (CUL): {CUL_kW:.3f} kW")
    else:
